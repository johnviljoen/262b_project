...
adding in multiple timesteps first before moving onto replacing optax
"""
...
def b_cost(pol_s, s):
    a = pol_inf(pol_s, s)
    s_kp1 = f(s, a)
    return (0.0001 * jnp.sum(a**2) + 10.0 * jnp.sum(s_kp1**2)) / s.shape[0]
...
for epoch in range(num_epochs):
    for initial_condition in train_data:  # shape = (1, 3333, 1, 2)
        ...
        loss, grads = jax.value_and_grad(b_cost)(pol_s, state)
