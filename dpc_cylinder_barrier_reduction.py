...
@functools.partial(jax.jit, static_argnums=(4,))
def mpc_cost(pol_s, s, cs, im_s, hzn, mu_bar):
    # DPC cost
    loss, Q, R, mu, b = 0, 5.0, 0.1, 1_000_000.0, s.shape[0]
    mu_pen = 1_000_000
    # mu_bar = 25_000
...
def step(step, opt_s, s, cs, im_s, hzn, opt_update, get_params, mu_bar):
    pol_s = get_params(opt_s)

    # loss, grads = jax.value_and_grad(cost)(pol_s, s, cs, im_s, hzn)
    loss, grads = jax.value_and_grad(mpc_cost)(pol_s, s, cs, im_s, hzn, mu_bar)
...
def test_violations(pol_s, s, cs):
    for i, t in enumerate(range(1000)):
        a = pol_inf(pol_s, s)
        s = f(s, a, cs)
        violation = posVel2Cyl(s, cs)[:,0].min() <= 0.0
        if violation == True:
            return True
    return violation
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        if i == 0:
            for e in range(ne):
                loss, opt_s, grads = step(e, opt_s, train_s, train_cs, train_im_s, hzn, opt_update, get_params, mu_bar)
        ...
        pol_s = get_params(best_opt_s)
        barrier_influence = barrier_cost_func(pol_s, s, cs, mu_bar=mu_bar)
        direction = barrier_influence / np.linalg.norm(barrier_influence)
        violations = test_violations(pol_s, train_s, train_cs)
        mu_alpha = 0.5 * mu_bar
        if violations == False:
            mu_bar -= mu_alpha * direction
            mu_bar_hist.append(mu_bar)
